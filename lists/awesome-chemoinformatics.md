...
**81 entries.** Every entry is free to use. Descriptions say what a tool *does*, not how good it is.
...
- [Pipelines & Workflow Tools](#pipelines--workflow-tools)
...
- [Indigo](https://lifescience.opensource.epam.com/indigo/) — Toolkit focused on correct handling of stereochemistry, tautomers, and Markush structures, with a good reaction engine. `c++` `java` `python`
...
- [RDKit](https://www.rdkit.org) — The foundational open cheminformatics toolkit: substructure search, conformers, descriptors, fingerprints, reaction handling, drawing. If a tutorial says "import rdkit", this is why. `c++` `java` `python`
...
- [InChI](https://www.inchi-trust.org) — IUPAC's canonical structure identifier and its hash, InChIKey; the closest thing chemistry has to a primary key. `c` `standard`
...
- [Galaxy ChemicalToolBox](https://github.com/bgruening/galaxytools) — Cheminformatics and docking tools packaged for the Galaxy platform, with reproducible histories and no local installation. `galaxy`
- [KNIME Analytics Platform](https://www.knime.com) — Visual workflow environment; the community and Vernalis extensions make it a full cheminformatics stack without writing code. Free desktop version. `java` `gui` `commercial-free-tier`
...
- [scikit-mol](https://github.com/EBjerrum/scikit-mol) — RDKit featurisers as scikit-learn transformers so molecules drop straight into a  with proper cross-validation. `python` `pipeline`
- [VEGA QSAR](https://www.vegahub.eu) — Free platform of validated QSAR models for toxicity and environmental endpoints, each reporting applicability domain and reliability. `java` `gui` `regulatory`
...
- [ASKCOS](https://github.com/ASKCOS/askcos-core) — MIT's synthesis planning suite: forward prediction, retrosynthesis, condition recommendation, and reaction feasibility. `python`
...
- [OpenMM](https://openmm.org) — GPU-accelerated molecular dynamics with a Python API and custom force expressions, usable as a library inside larger workflows. `c++` `python` `gpu`
...
- [3Dmol.js](https://3dmol.csb.pitt.edu) — WebGL molecular viewer that embeds in any page with a few lines of JavaScript; ideal for teaching pages and notebooks. `javascript`
...
- [Mol*](https://molstar.org) — The viewer now behind RCSB PDB and PDBe; handles very large structures and volumetric data in the browser. `typescript`
...
- [PyMOL (open-source build)](https://github.com/schrodinger/pymol-open-source) — The community build of PyMOL: publication-quality rendering, scriptable in Python. `c` `python`
...
- [Polaris](https://polarishub.io) — Benchmark platform emphasising realistic splits and error analysis over leaderboard chasing. `python` `benchmark`
- [Therapeutics Data Commons](https://tdcommons.ai) — Curated ML tasks across ADMET, HTS, and synthesis, with leaderboards and consistent splits. `python` `benchmark`
...
- [RDKit Discussions](https://github.com/rdkit/rdkit/discussions) — Where the maintainers answer; searchable and consistently high quality. `forum` `methodology`
