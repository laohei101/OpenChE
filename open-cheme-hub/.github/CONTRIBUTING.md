...
Edit the `README.md` of the relevant list directly in the GitHub web editor — the pencil icon
does everything, including forking.

**Format.** One line, in the section it belongs to, alphabetical within the section:

```markdown
- [Cantera](https://cantera.org) — Object-oriented toolkit for chemical kinetics, thermodynamics, and transport, with reactor network solvers. `python` `c++` `matlab`
```
