...
#
# * THIS SCRIPT IS NOT THE DEFAULT DEPLOYMENT PATH. The canonical home for
#   OpenChemE is the single root repository. Splitting into nine repositories
#   is deferred until the catalog has independent maintainers and enough
#   per-area issue traffic to justify the coordination cost. See ROADMAP.md.
#
# * THE DEFAULT ORG IS `OpenChemE` and every cross-reference in the committed
#   files already points there. To publish elsewhere, pass --org AND
#   --rewrite-urls, or your copy will link back to an org you do not control.
#
# * CONTACT ADDRESS is yu.dai@mail.utoronto.ca throughout (CODE_OF_CONDUCT.md,
#   SECURITY.md, SUPPORT.md). Change it in those files if a different inbox
#   should receive conduct and security reports.
#
